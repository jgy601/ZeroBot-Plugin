...
func init() {
	...
	engine, gameManager, err := gamesystem.Register("猜硬币", &gamesystem.GameInfo{
		Command: "- 创建猜硬币\n" +
			"- [加入|开始]游戏\n" +
			"- 开始投币",
		Help: "每个人宣言硬币正面数量后,掷出参游人数的硬币",
		Rewards: "正面与宣言的数量相同的场合获得 正面数*10 金币\n" +
			"正面与宣言的数量相差2以内的场合获得 正面数*5 金币\n" +
			"其他的的场合失去 10 金币",
	})
	if err != nil {
		panic(err)
	...
	engine.OnFullMatch("创建猜硬币", zero.OnlyGroup, func(ctx *zero.Ctx) bool {
		...
		ctx.SendChain(message.Text("OK,我要开始投掷硬币了～"))
		process.SleepAbout1sTo2s()
		ctx.SendChain(message.Text("一共投掷了", len(duel), "枚硬币,其中正面的有", positive, "枚正面。\n具体结果如下", result))
		// 数据结算
