...
func drawScore16(a *scdata) (image.Image, error) {
	...
	canvas.DrawStringAnchored("金币 + "+strconv.Itoa(a.inc), 350, 350, 0, 0)
	canvas.DrawStringAnchored("当前金币："+strconv.Itoa(a.score), 350, 400, 0, 0)
...
func drawScore15(a *scdata) (image.Image, error) {
	...
	canvas.DrawString(a.nickname+fmt.Sprintf(" 金币+%d", a.inc), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.3)
	canvas.DrawString("当前金币:"+strconv.FormatInt(int64(a.score), 10), float64(back.Bounds().Size().X)*0.1, float64(back.Bounds().Size().Y)*1.4)
