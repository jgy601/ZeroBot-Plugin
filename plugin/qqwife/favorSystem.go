...
func init() {
	...
	engine.OnRegex(`^买礼物给\s?(\[CQ:at,qq=(\d+)\]|(\d+))`, getdb).SetBlock(true).Limit(ctxext.LimitByUser).
		Handle(func(ctx *zero.Ctx) {
			...
			moneyToFavor := rand.Intn(math.Min(walletinfo, 50)) + 1
			// 计算钱对应的好感值
			newFavor := 1
			moodMax := 3
			if favor > 50 {
			...
			// 输出结果
			if mood == 0 {
				ctx.SendChain(message.Text("你花了", moneyToFavor, "金币买了一件女装送给了ta,ta很不喜欢,你们的好感度降低至", lastfavor))
			} else {
				ctx.SendChain(message.Text("你花了", moneyToFavor, "金币买了一件女装送给了ta,ta很喜欢,你们的好感度升至", lastfavor))
			}
		})
